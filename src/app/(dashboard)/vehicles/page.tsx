...
import {
  ...
  Search, Wrench, Package, ShieldCheck, Gauge, Fuel
} from 'lucide-react';
...
export default function VehiclesPage() {
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    setShowAdd(false);
    setForm({ vehicleNo: '', type: 'Heavy Duty Truck', capacity: 10000, location: 'Central Distribution Hub', lastService: '' });
  };
  ...
  return (
    <div className="animate-slide-in">
      ...
      {/* Maintenance alert ? watermelon Alert pattern */}
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    <div className="mono" style={{ fontWeight: 800, fontSize: 17, color: 'var(--text-high)' }}>
                ...
                <span className={`badge ${getStatusBadgeClass(selectedVehicle.status)}`} style={{ fontSize: 12 }}>
              ...
              <div className="grid-2" style={{ gap: 10, marginBottom: 16 }}>
                <div className="hud-gauge">
                  ...
                  <div className="hud-gauge-value" style={{ color: 'var(--brand)' }}>{selectedVehicle.fuelLevel || 88}%</div>
                </div>
                <div className="hud-gauge">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12.5, marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border)', paddingBottom: 6 }}>
                  <span style={{ color: 'var(--text-low)' }}>Rated Payload Capacity</span>
                  <span className="mono" style={{ fontWeight: 700, color: 'var(--text-high)' }}>{selectedVehicle.capacity.toLocaleString()} kg</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border)', paddingBottom: 6 }}>
                  <span style={{ color: 'var(--text-low)' }}>Current Assigned Load</span>
                  <span className="mono" style={{ fontWeight: 700, color: selectedVehicle.currentLoad > 0 ? 'var(--brand)' : 'var(--text-low)' }}>
                    {selectedVehicle.currentLoad.toLocaleString()} kg
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border)', paddingBottom: 6 }}>
                  <span style={{ color: 'var(--text-low)' }}>Stationed Hub Location</span>
                  <span style={{ fontWeight: 600, color: 'var(--text-high)' }}>{selectedVehicle.location}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-low)' }}>Last Service Inspection</span>
                  <span style={{ color: 'var(--text-mid)' }}>{selectedVehicle.lastService || '2026-08-15'}</span>
                </div>
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                <LabeledProgress
                  progress={Math.round((selectedVehicle.currentLoad / selectedVehicle.capacity) * 100)}
                  labels={[
                    `Capacity: ${selectedVehicle.currentLoad.toLocaleString()} / ${selectedVehicle.capacity.toLocaleString()} kg`,
                    `${Math.round((selectedVehicle.currentLoad / selectedVehicle.capacity) * 100)}% utilized`,
                  ]}
                  height={8}
                  intervalMs={3000}
                />
